<?php
/*
 * Breadcrumbs *** YAMS-ified version ***
 *
 ...
 * Version: 1.0.1
 *
 * Modified by PMS to work with YAMS
 */
if ( !isset( $modx ) || ! $modx->isFrontend() )
{
  return '';
}
...
if ( $showCurrentCrumb )
{
  ...
  {
//    $tvs = $modx->getTemplateVarOutput(
//      $yamsMultiTVNameArray
//        , $modx->documentIdentifier
//      );
    $crumbs[] = array(
        'id' => $modx->documentObject['id'],
        'parent' => $modx->documentObject['parent'],
        'pagetitle' => '((yams_data:' . $modx->documentObject['id'] . ':pagetitle_' . $langid . '))',
        'longtitle' => '((yams_data:' . $modx->documentObject['id'] . ':longtitle_' . $langid . '))',
        'menutitle' => '((yams_data:' . $modx->documentObject['id'] . ':menutitle_' . $langid . '))',
        'description' => '((yams_data:' . $modx->documentObject['id'] . ':description_' . $langid . '))'
      );
  }
  else
  {
...
while ( $parent && $loopSafety < 1000 )
{
    ...
    {
        // Add crumb
        if ( $yams->IsMultilingualDocument( $tempCrumb['id'] ) )
        {
//          $tvs = $modx->getTemplateVarOutput(
//            $yamsMultiTVNameArray
//              , $tempCrumb['id']
//            );
          $crumbs[] = array(
            ...
            'pagetitle' => '((yams_data:' . $tempCrumb['id'] . ':pagetitle_' . $langid . '))',
            'longtitle' => '((yams_data:' . $tempCrumb['id'] . ':longtitle_' . $langid . '))',
            'menutitle' => '((yams_data:' . $tempCrumb['id'] . ':menutitle_' . $langid . '))',
            'description' => '((yams_data:' . $tempCrumb['id'] . ':description_' . $langid . '))'
            );
        }
        else
        {
...
{
    if ( $yams->IsMultilingualDocument( $homeCrumb['id'] ) )
    {
//      $tvs = $modx->getTemplateVarOutput(
//        $yamsMultiTVNameArray
//          , $homeCrumb['id']
//        );
      $crumbs[] = array(
        ...
        'pagetitle' => '((yams_data:' . $homeCrumb['id'] . ':pagetitle_' . $langid . '))',
        'longtitle' => '((yams_data:' . $homeCrumb['id'] . ':longtitle_' . $langid . '))',
        'menutitle' => '((yams_data:' . $homeCrumb['id'] . ':menutitle_' . $langid . '))',
        'description' => '((yams_data:' . $homeCrumb['id'] . ':description_' . $langid . '))'
        );
    }
    else
    {
...
foreach ( $crumbs as $c )
{
    ...
    )
    {
        ...
        $pretemplateCrumb .= '<a class="'.$crumbClass.'" href="'
          // . $modx->makeUrl($c['id'])
          . $yams->ConstructResolvedURL( $langid, $c['id'] )
          // . '(yams_docr:' . $c['id'] . ')'
          . '" title="'.$title.'">'.$text.'</a>';
    }
    else
